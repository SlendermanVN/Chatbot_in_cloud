...
<div class="row">
    <div class="col-12 mt-2">
        <form action="<?= BASE_URL ?>/index.php?route=admin_product_update&id=<?= $product['id'] ?>" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                    ...
                    <div class="card mb-4">
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="name" name="name" required value="<?= htmlspecialchars($product['name']) ?>" placeholder="Nhập tên sản phẩm..." class="form-control">
                            </div>
                            
                            <div class="form-group">
                                <label for="description" class="font-weight-bold">Mô tả sản phẩm</label>
                                <textarea id="description" name="description" rows="5" placeholder="Mô tả chi tiết..." class="form-control"><?= htmlspecialchars($product['description']) ?></textarea>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" id="sku" name="sku" value="<?= htmlspecialchars($product['sku']) ?>" class="form-control">
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="stock" class="font-weight-bold">Tồn kho hiện tại</label>
                                    <input type="number" id="stock" name="stock" min="0" value="<?= $product['stock'] ?>" class="form-control">
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <div class="input-group">
                                        <input type="number" id="price" name="price" required min="0" value="<?= $product['price'] ?>" class="form-control">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <div class="input-group">
                                        <input type="number" id="sale_price" name="sale_price" min="0" value="<?= $product['sale_price'] ?>" class="form-control">
                                        <div class="input-group-append">
                                            <span class="input-group-text">VNĐ</span>
                                        </div>
                                    </div>
                ...
                <div class="col-lg-4">
                    ...
                    <div class="card mb-4">
                        <div class="card-body">
                            ...
                            <?php if (!empty($product['images'])): ?>
                                <div class="mb-3">
                                    <label class="font-weight-bold text-muted text-uppercase" style="font-size: 12px;">Ảnh đang có</label>
                                    <div class="row no-gutters">
                                        <?php foreach ($product['images'] as $img): ?>
                                            <div class="col-3 p-1">
                                                <div class="position-relative border rounded bg-light" style="padding-bottom: 100%; overflow: hidden;">
                                                    <img src="<?= BASE_URL ?>/<?= htmlspecialchars($img['image_path']) ?>" 
                                                         alt="Ảnh sản phẩm"
                                                         class="position-absolute" style="width: 100%; height: 100%; object-fit: cover;">
                                                    
                                                    <!-- Nút Xóa (Dấu X) -->
                                                    <form action="<?= BASE_URL ?>/index.php?route=admin_product_delete_image" method="POST" class="position-absolute" style="top: 5px; right: 5px; z-index: 20;" onsubmit="return confirm('Xóa ảnh này?')">
                                                        <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
                                                        <input type="hidden" name="image_id" value="<?= $img['id'] ?>">
                                                        <input type="hidden" name="product_id" value="<?= $product['id'] ?>">
                                                        <button type="submit" class="btn btn-danger btn-sm p-0 d-flex align-items-center justify-content-center shadow-sm" style="width: 22px; height: 22px; border-radius: 50%; border: 1px solid white;">
                                                    ...
                                                    <?php if(!$img['is_primary']): ?>
                                                        <form action="<?= BASE_URL ?>/index.php?route=admin_product_set_primary" method="POST" class="position-absolute w-100" style="bottom: 0; left: 0; z-index: 10;">
                                                            <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
                                                            <input type="hidden" name="image_id" value="<?= $img['id'] ?>">
                                                            <input type="hidden" name="product_id" value="<?= $product['id'] ?>">
                                                            <button type="submit" class="btn btn-dark btn-block btn-xs py-1" style="font-size: 9px; opacity: 0.8; border-radius: 0; border: none;" title="Đặt làm ảnh chính">
                                                                <i class="fa-solid fa-star"></i> SET MAIN
                                                            </button>
                                                        </form>
                                                    <?php else: ?>
                                                        <div class="position-absolute fixed-bottom bg-primary text-white text-center py-1 font-weight-bold" style="font-size: 9px; opacity: 0.9;">
                            ...
                            <div class="form-group mt-3">
                                ...
                                <div class="custom-file mb-2">
                                    <input type="file" class="custom-file-input" id="images" name="images[]" multiple accept=".jpg,.jpeg,.png,.webp,.gif">
                    ...
                    <!-- Submit Button -->
                    <button type="submit" class="btn btn-primary btn-block btn-lg"><i class="fa-solid fa-floppy-disk"></i> LƯU THAY ĐỔI</button>
